...
type Store interface {
	CheckUniqueLogin(ctx context.Context, login string) error
	SaveDataNewUser(ctx context.Context, login, password, key string, userID uint32) error
	GetSecretKey(ctx context.Context, userID uint32) (string, error)
	GetUserID(ctx context.Context, login string) (uint32, error)
...
func NewConnect(dsn string) (*UserDB, error) {
	...
	_, err = db.Exec(`CREATE TABLE IF NOT EXISTS registration
					(login text NOT NULL, 
...
func (userdb *UserDB) SaveDataNewUser(ctx context.Context, login, password, key string, userID uint32) error {
	_, err := userdb.db.ExecContext(ctx, "INSERT INTO registration(login, password, secretkey, userID)"+
		"values($1, $2, $3, $4)", login, password, key, userID)
	return err
}
